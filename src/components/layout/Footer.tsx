...
function Footer({ brand, linkGroups, address, phone, email, openingHours, socials }: FooterProps) {
    ...
    <footer className="footer">
      <div className="container footer-grid">
        ...
        <div className="footer-col brand-info">
          <div className="footer-brand">
            ...
            <p className="brand-desc">{t(brand.description)}</p>
          </div>
        ...
        <div className="footer-col contact-info">
          ...
          <address>
            ...
            {phone && <p><a href={`tel:${phone.replace(/\s/g, '')}`}>{t(phone)}</a></p>}
            {email && <p><a href={`mailto:${email}`}>{t(email)}</a></p>}
          </address>
          
          <div className="opening-hours-section">
            <h4>{t('contact.info.hours')}</h4>
            <p className="opening-hours-text">{openingHours && t(openingHours)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="footer-bottom">
        <div className="container">
          <p>&copy; {new Date().getFullYear()} {brand.logoText} {brand.logoHighlight}. All rights reserved.</p>
        </div>
      </div>
    </footer>
  )
